...
// their values.
//
AP_EscMonitor::AP_EscMonitor(esc_failsafe_handler_fn_t battery_failsafe_handler_fn) :
...
AP_EscMonitor::init()
{
    ...
    if (_params[0]._type == 0) {
        ...
        _params[0]._type.set((AP_EscMonitor_Params::BattMonitor_Type)HAL_BATT_MONITOR_DEFAULT);
    }
#endif
    ...
    for (uint8_t instance=0; instance<AP_BATT_MONITOR_MAX_INSTANCES; instance++) {
        ...
        switch (get_type(instance)) {
            case AP_EscMonitor_Params::BattMonitor_TYPE_ANALOG_VOLTAGE_ONLY:
            case AP_EscMonitor_Params::BattMonitor_TYPE_ANALOG_VOLTAGE_AND_CURRENT:
                drivers[instance] = new AP_EscMonitor_Analog(*this, state[instance], _params[instance]);
                break;
#if HAL_BATTMON_SMBUS_ENABLE
            case AP_EscMonitor_Params::BattMonitor_TYPE_SOLO:
                _params[instance]._i2c_bus.set_default(AP_EscMONITOR_SMBUS_BUS_INTERNAL);
                drivers[instance] = new AP_EscMonitor_SMBus_Solo(*this, state[instance], _params[instance],
                                                                  hal.i2c_mgr->get_device(_params[instance]._i2c_bus, AP_EscMONITOR_SMBUS_I2C_ADDR,
                                                                                          100000, true, 20));
                break;
            case AP_EscMonitor_Params::BattMonitor_TYPE_SUI3:
                _params[instance]._i2c_bus.set_default(AP_EscMONITOR_SMBUS_BUS_INTERNAL),
                drivers[instance] = new AP_EscMonitor_SMBus_SUI(*this, state[instance], _params[instance],
                                                                 hal.i2c_mgr->get_device(_params[instance]._i2c_bus, AP_EscMONITOR_SMBUS_I2C_ADDR,
                                                                                          100000, true, 20), 3);
                break;
            case AP_EscMonitor_Params::BattMonitor_TYPE_SUI6:
                _params[instance]._i2c_bus.set_default(AP_EscMONITOR_SMBUS_BUS_INTERNAL),
                drivers[instance] = new AP_EscMonitor_SMBus_SUI(*this, state[instance], _params[instance],
                                                                 hal.i2c_mgr->get_device(_params[instance]._i2c_bus, AP_EscMONITOR_SMBUS_I2C_ADDR,
                                                                                         100000, true, 20), 6);
                break;
            case AP_EscMonitor_Params::BattMonitor_TYPE_MAXELL:
                _params[instance]._i2c_bus.set_default(AP_EscMONITOR_SMBUS_BUS_EXTERNAL);
                drivers[instance] = new AP_EscMonitor_SMBus_Maxell(*this, state[instance], _params[instance],
                                                                    hal.i2c_mgr->get_device(_params[instance]._i2c_bus, AP_EscMONITOR_SMBUS_I2C_ADDR,
                                                                                            100000, true, 20));
                break;
#endif // HAL_BATTMON_SMBUS_ENABLE
            case AP_EscMonitor_Params::BattMonitor_TYPE_BEBOP:
#if CONFIG_HAL_BOARD_SUBTYPE == HAL_BOARD_SUBTYPE_LINUX_BEBOP || CONFIG_HAL_BOARD_SUBTYPE == HAL_BOARD_SUBTYPE_LINUX_DISCO
                drivers[instance] = new AP_EscMonitor_Bebop(*this, state[instance], _params[instance]);
#endif
                break;
            case AP_EscMonitor_Params::BattMonitor_TYPE_UAVCAN_BatteryInfo:
#if HAL_WITH_UAVCAN
                drivers[instance] = new AP_EscMonitor_UAVCAN(*this, state[instance], AP_EscMonitor_UAVCAN::UAVCAN_BATTERY_INFO, _params[instance]);
#endif
                break;
            case AP_EscMonitor_Params::BattMonitor_TYPE_BLHeliESC:
#ifdef HAVE_AP_BLHELI_SUPPORT
                drivers[instance] = new AP_EscMonitor_BLHeliESC(*this, state[instance], _params[instance]);
#endif
                break;
            case AP_EscMonitor_Params::BattMonitor_TYPE_Sum:
                drivers[instance] = new AP_EscMonitor_Sum(*this, state[instance], _params[instance], instance);
                break;
#if HAL_BATTMON_FUEL_ENABLE
            case AP_EscMonitor_Params::BattMonitor_TYPE_FuelFlow:
                drivers[instance] = new AP_EscMonitor_FuelFlow(*this, state[instance], _params[instance]);
                break;
            case AP_EscMonitor_Params::BattMonitor_TYPE_FuelLevel_PWM:
                drivers[instance] = new AP_EscMonitor_FuelLevel_PWM(*this, state[instance], _params[instance]);
                break;
#endif // HAL_BATTMON_FUEL_ENABLE
            case AP_EscMonitor_Params::BattMonitor_TYPE_NeoDesign:
                _params[instance]._i2c_bus.set_default(AP_EscMONITOR_SMBUS_BUS_INTERNAL),
                drivers[instance] = new AP_EscMonitor_SMBus_NeoDesign(*this, state[instance], _params[instance],
                                                                 hal.i2c_mgr->get_device(_params[instance]._i2c_bus, AP_EscMONITOR_SMBUS_I2C_ADDR,
                                                                                         100000, true, 20));
                break;
            case AP_EscMonitor_Params::BattMonitor_TYPE_NONE:
            default:
                break;
        }
...
void AP_EscMonitor::convert_params(void) {
    ...
    for (uint8_t i = 0; i < ARRAY_SIZE(conversionTable); i++) {
        ...
        info.type = (ap_var_type)AP_EscMonitor_Params::var_info[destination_index].type;
        if (param_instance) {
            hal.util->snprintf(param_name, sizeof(param_name), "BATT2_%s", AP_EscMonitor_Params::var_info[destination_index].name);
        } else {
            hal.util->snprintf(param_name, sizeof(param_name), "BATT_%s", AP_EscMonitor_Params::var_info[destination_index].name);
        }
...
AP_EscMonitor::read()
{
    for (uint8_t i=0; i<_num_instances; i++) {
        if (drivers[i] != nullptr && _params[i].type() != AP_EscMonitor_Params::BattMonitor_TYPE_NONE) {
            drivers[i]->read();
...
bool AP_EscMonitor::healthy(uint8_t instance) const {
    return instance < _num_instances && state[instance].healthy;
}

/// voltage - returns battery voltage in volts
float AP_EscMonitor::voltage(uint8_t instance) const
{
...
float AP_EscMonitor::voltage_resting_estimate(uint8_t instance) const
{
...
bool AP_EscMonitor::current_amps(float &current, uint8_t instance) const {
...
bool AP_EscMonitor::consumed_mah(float &mah, const uint8_t instance) const {
    if ((instance < _num_instances) && (drivers[instance] != nullptr) && drivers[instance]->has_current()) {
...
bool AP_EscMonitor::consumed_wh(float &wh, const uint8_t instance) const {
...
uint8_t AP_EscMonitor::capacity_remaining_pct(uint8_t instance) const
{
    if (instance < _num_instances && drivers[instance] != nullptr) {
...
int32_t AP_EscMonitor::pack_capacity_mah(uint8_t instance) const
{
...
void AP_EscMonitor::check_failsafes(void)
{
    ...
    if (hal.util->get_soft_armed()) {
        for (uint8_t i = 0; i < _num_instances; i++) {
            ...
            switch (type) {
                case AP_EscMonitor::BatteryFailsafe_None:
                    continue; // should not have been called in this case
                case AP_EscMonitor::BatteryFailsafe_Low:
                    action = _params[i]._failsafe_low_action;
                    type_str = "low";
                    break;
                case AP_EscMonitor::BatteryFailsafe_Critical:
...
bool AP_EscMonitor::overpower_detected() const
{
    bool result = false;
...
bool AP_EscMonitor::overpower_detected(uint8_t instance) const
{
#if APM_BUILD_TYPE(APM_BUILD_ArduPlane)
...
bool AP_EscMonitor::has_cell_voltages(const uint8_t instance) const
{
    if (instance < _num_instances && drivers[instance] != nullptr) {
...
const AP_EscMonitor::cells & AP_EscMonitor::get_cell_voltages(const uint8_t instance) const
{
    if (instance >= AP_BATT_MONITOR_MAX_INSTANCES) {
...
bool AP_EscMonitor::get_temperature(float &temperature, const uint8_t instance) const
{
    if (instance >= AP_BATT_MONITOR_MAX_INSTANCES) {
        return false;
    } else {
...
bool AP_EscMonitor::get_cycle_count(uint8_t instance, uint16_t &cycles) const
{
...
bool AP_EscMonitor::arming_checks(size_t buflen, char *buffer) const
{
...
void AP_EscMonitor::checkPoweringOff(void)
{
    for (uint8_t i = 0; i < _num_instances; i++) {
...
*/
bool AP_EscMonitor::reset_remaining(uint16_t battery_mask, float percentage)
{
    bool ret = true;
